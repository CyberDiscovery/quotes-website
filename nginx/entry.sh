...
# Get certificate
sed -i -r 's/(listen .*443)/\1;#/g; s/(ssl_(certificate|certificate_key|trusted_certificate) )/#;#\1/g' /etc/nginx/sites-available/nginx-site.conf 
...
sed -i -r 's/#?;#//g' /etc/nginx/sites-available/nginx-site.conf
nginx -t && systemctl reload nginx 
